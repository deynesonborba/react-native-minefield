const createBoard = (rows, columns) => {
  //Array(n): create an array with n size
  //.fill(n): fill all elements of the array with n
  //.map(): make something with array based on parameters
  //.map() receive two params, fist the element, second the index
  //.map() use _ to ignore the element
  return Array(rows).fill(0).map( ( _, row) => {
    return Array(columns).fill(0).map( (_, column) => {
      return {
        //1st row to declaration of variavle, 2nd row is the value
        row: row, //literal form
        column, //ecmascript form
        opened: false,
        flagged: false,
        mined: false,
        exploded: false,
